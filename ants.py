...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt and insect.imposter == False:
                return
...
class Bee(Insect):
    ...
    # add instance variable, been_scared=False
    def __init__(self, armor, place=None):
        """Create an Insect with an ARMOR amount and a starting PLACE."""
        
        Insect.__init__(self, armor,place)
        self.been_scared=False
    ...
    def blocked(self):
        ...
        #return self.place.ant is not None    #Given code
        # END Problem 7
    ...
    def action(self, colony):
        ...
        if self.been_scared==True:   #Revert to True if needed
            a=self.place
            a=a.entrance       
            destination=a                   
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        tracker = 0
        while next_place != hive:
            if next_place.bees != [] and (self.max_range >= tracker and self.min_range < tracker):
                return random_or_none(next_place.bees)
            tracker += 1
            next_place = next_place.entrance
        return None

        #while p!=hive and self.max_range
        #count<max_range:
...
class ShortThrower(ThrowerAnt):
    ...
    max_range, min_range = 3, 0




    #how do you save min_range and max_range,d does place have a numerical value?
    #ShortThrower.nearest_bee(max_range)
    # END Problem 4
...
class LongThrower(ThrowerAnt):
    ...
    food_cost=2
    #min_range=5
    min_range, max_range = 4, float('inf')
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    #LongThrower.nearest_bee(min_range)
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0:
            ...
            for x in copy:
                Insect.reduce_armor(x,self.damage)  #Super.reduce_armor(x,3)

            copy=a  # is this needed?
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting>0:
            ...
            a=self.place.bees
            if len(a)>0:
                b=random.choice(a)   #####erroring here
                #bee.armor=0
                b.armor=0
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if QueenAnt.imposter == False:
            self.imposter = False
            QueenAnt.imposter = True
        else:
            self.imposter = True
        self.armor=armor
        #self.Queen_num=self.Queen_num+1
        #self.num=1+self.Queen_num
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        next_exit = self.place.exit
        if self.imposter == True:
            self.reduce_armor(self.armor)
            return
        ScubaThrower.action(self, colony)
        while next_exit is not None:
            if next_exit.ant is not None:
                if next_exit.ant.is_container and next_exit.ant.contained_ant is not None:
                    if next_exit.ant.contained_ant not in self.record_of_ants:
                        next_exit.ant.contained_ant.damage = next_exit.ant.contained_ant.damage*2
                        self.record_of_ants.append(next_exit.ant.contained_ant)
                if next_exit.ant not in self.record_of_ants:
                    next_exit.ant.damage=next_exit.ant.damage*2
                    self.record_of_ants.append(next_exit.ant)

            next_exit=next_exit.exit
...
def make_scare(action, bee):
    ...
    def action1(bee, way):
        if way == True:
            bee.place = bee.exicolonyt
            way = False
        elif way == False:
            bee.place = bee.entrance
            way = True
    return action1'''
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    copy=bee.action
    tracker=1
    call_func=effect(bee.action,bee)
