...
class Bee(Insect):
    ...
    #    """Create an Bee with a scared attribute."""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        #Urjils code
        p=self.place
        while p!=hive:    #p.entrance  #AntColony.
            if len(p.bees)>0:                           # = []        # A list of Bees
                return random_or_none(p.bees)  #self.place.bees
            else:
                p=p.entrance
...
class ShortThrower(ThrowerAnt):
    ...
    max_range, min_range = 3, 0   #Rahuls code
...
class LongThrower(ThrowerAnt):
    ...
    food_cost=2
    #min_range=5
    min_range, max_range = 4, float('inf')   #Rahuls code
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        #How to check for imposter? do we need isinstance?
    if Queen_num<2:
        do regular action of doubling damage
        #no problem
    else:
        if self.num>1:
            Insect.reduce_armor(ant,armour)
